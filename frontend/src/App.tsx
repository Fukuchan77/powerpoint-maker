...
function App() {
  ...
  const [extractedContent, setExtractedContent] =
    useState<ContentExtractionResult | null>(null);
  ...
  const convertedExtractedSlides: SlideContent[] | undefined =
    extractedContent?.slides.map((slide) => ({
    ...
    <ErrorBoundary>
      <div>
        ...
        {extractedContent && (
          <div
            className="card"
            style={{ backgroundColor: "#f0fdf4", marginBottom: "1rem" }}
          >
            <h3 style={{ margin: 0 }}>Content Extracted</h3>
            <p style={{ margin: 0 }}>
              Source: {extractedContent.filename} (
              {extractedContent.slides.length} slides)
            </p>
