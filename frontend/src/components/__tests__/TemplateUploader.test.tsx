...
describe('TemplateUploader', () => {
  ...
  describe('Mode Selection', () => {
    it('defaults to template mode', () => {
      ...
      const templateRadio = screen.getByLabelText('Template Only') as HTMLInputElement;
    ...
    it('switches to content mode when selected', async () => {
      ...
      const contentRadio = screen.getByLabelText('Extract Content') as HTMLInputElement;
  ...
  describe('File Upload - Template Mode', () => {
    ...
    it('displays loading state during template analysis', async () => {
      vi.mocked(axios.post).mockImplementation(
        () =>
          new Promise((resolve) =>
            setTimeout(() => resolve({ data: { filename: 'test.pptx', masters: [] } }), 100)
  ...
  describe('Error Handling', () => {
    it('displays error for failed template analysis', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Failed to analyze template/i)).toBeInTheDocument();
      });
    });

    it('displays error for failed content extraction', async () => {
      vi.mocked(axios.post).mockRejectedValueOnce(new Error('Extraction failed'));
      ...
      await waitFor(() => {
        expect(screen.getByText(/Failed to extract content/i)).toBeInTheDocument();
      });
    });
  });
