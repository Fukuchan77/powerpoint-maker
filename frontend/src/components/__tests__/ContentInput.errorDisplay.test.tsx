  ...
  describe('Structured Syntax Error Display', () => {
    it('displays structured error for MARKDOWN_SYNTAX_ERROR', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText("No slides found. Use '## Heading' to create slides.")
        ).toBeInTheDocument();
        expect(screen.getByText(/Location: Line 1, Column 1/)).toBeInTheDocument();
      });
    });
    ...
    it('displays line and column information correctly', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Location: Line 5, Column 12/)).toBeInTheDocument();
      });
    });
    ...
    it('shows generic error for non-syntax errors', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.queryByText('Markdown Syntax Error')).not.toBeInTheDocument();
      });
    });
    ...
    it('clears generic error when user starts typing', async () => {
      ...
      vi.mocked(pptxApi.parseMarkdown).mockRejectedValueOnce(new Error('Network error'));
      ...
      await waitFor(() => {
        expect(screen.queryByText(/Failed to parse Markdown/i)).not.toBeInTheDocument();
      });
    });
  });
});
