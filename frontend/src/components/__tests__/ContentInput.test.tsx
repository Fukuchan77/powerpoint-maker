...
describe('ContentInput', () => {
  ...
  describe('Basic Rendering', () => {
    ...
    it('defaults to web_search source', () => {
      ...
      const webSearchRadio = screen.getByLabelText('Web Search') as HTMLInputElement;
  ...
  describe('Source Selection', () => {
    it('switches to markdown source when selected', async () => {
      ...
      expect(screen.getByPlaceholderText(/# Presentation Title/)).toBeInTheDocument();
    });
  ...
  describe('Extracted Content Support', () => {
    it('shows extracted content option when extractionId provided', () => {
      const mockSlides = [{ layout_index: 0, title: 'Test Slide', bullet_points: [] }];
    ...
    it('does not show extracted content option without extractionId', () => {
      ...
      expect(screen.queryByLabelText('Extracted Content')).not.toBeInTheDocument();
    });

    it('does not show extracted content option without extractedSlides', () => {
      render(<ContentInput onContentGenerated={mockOnContentGenerated} extractionId="test-id" />);
    ...
    it('displays slide count for extracted content', async () => {
      ...
      expect(screen.getByText(/Using 3 slides from extracted PPTX content/)).toBeInTheDocument();
    });
  ...
  describe('Markdown Parsing', () => {
    it('calls parseMarkdown when button clicked', async () => {
      ...
      await waitFor(() => {
        expect(pptxApi.parseMarkdown).toHaveBeenCalledWith('# Test\n\n## Slide 1\n- Point A');
        expect(mockOnContentGenerated).toHaveBeenCalledWith(mockResponse.slides);
      });
    });
    ...
    it('displays error on parsing failure', async () => {
      ...
      vi.mocked(pptxApi.parseMarkdown).mockRejectedValueOnce(new Error('Parse error'));
      ...
      await waitFor(() => {
        expect(screen.getByText(/Failed to parse Markdown/i)).toBeInTheDocument();
      });
  ...
  describe('Textarea Interaction', () => {
    it('updates markdown content on textarea change', async () => {
      ...
      const textarea = screen.getByPlaceholderText(/# Presentation Title/) as HTMLTextAreaElement;
  ...
  describe('Text Input', () => {
    ...
    it('calls generateFromText API when Generate Slides clicked', async () => {
      ...
      vi.mocked(layoutIntelligenceApi.generateFromText).mockResolvedValueOnce(mockResponse);
      ...
      const textarea = screen.getByTestId('text-input-textarea');
      await userEvent.type(textarea, 'Our company achieved 50% growth this year.');
    ...
    it('enforces 10,000 character limit on textarea', async () => {
      ...
      const textarea = screen.getByTestId('text-input-textarea') as HTMLTextAreaElement;
    ...
    it('displays timeout error message', async () => {
      ...
      vi.mocked(layoutIntelligenceApi.generateFromText).mockRejectedValueOnce(timeoutError);
    ...
    it('displays 504 error message', async () => {
      ...
      vi.mocked(layoutIntelligenceApi.generateFromText).mockRejectedValueOnce(error as never);
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Processing took too long. Please try with shorter or simpler content./)
        ).toBeInTheDocument();
      });
    ...
    it('clears error when user starts typing', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Failed to generate slides/)).toBeInTheDocument();
      });

      // Type more content - error should clear
      await userEvent.type(textarea, ' more');

      expect(screen.queryByText(/Failed to generate slides/)).not.toBeInTheDocument();
