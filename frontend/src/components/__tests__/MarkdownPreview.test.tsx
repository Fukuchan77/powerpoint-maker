...
describe('MarkdownPreview', () => {
  ...
  describe('Debouncing', () => {
    it('debounces parsing with default delay', async () => {
      const mockResponse = {
        ...
        slides: [{ layout_index: 0, title: 'Slide 1', bullet_points: ['Point 1'] }],
        warnings: [],
      };
      ...
      render(
        <MarkdownPreview markdownContent="# Test\n\n## Slide 1\n- Point 1" debounceMs={100} />
      );
  ...
  describe('Slide Display', () => {
    ...
    it('displays slide with image', async () => {
      ...
      await waitFor(
        () => {
          expect(screen.getByText(/Image: https:\/\/example.com\/image.jpg/)).toBeInTheDocument();
        },
        { timeout: 300 }
      );
    });
    ...
    it('shows message when no slides detected', async () => {
      ...
      render(<MarkdownPreview markdownContent="# Just a title" debounceMs={50} />);
  ...
  describe('Warnings Display', () => {
    it('displays warnings from parser', async () => {
      ...
      render(<MarkdownPreview markdownContent="# Test\n\n## Slide 1" debounceMs={50} />);

      await waitFor(
        () => {
          expect(screen.getByText(/Warning 1: Invalid URL/)).toBeInTheDocument();
        },
        { timeout: 300 }
      );
  ...
  describe('Error Handling', () => {
    it('handles parse errors gracefully', async () => {
      vi.mocked(pptxApi.parseMarkdown).mockRejectedValue(new Error('Parse failed'));
    ...
    it('clears slides on error', async () => {
      ...
      const { rerender } = render(<MarkdownPreview markdownContent="# Valid" debounceMs={50} />);
      ...
      await waitFor(
        () => {
          expect(screen.queryByText('1 slide detected')).not.toBeInTheDocument();
        },
        { timeout: 300 }
      );
