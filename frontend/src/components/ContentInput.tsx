...
function SyntaxErrorDisplay({ error }: { error: MarkdownSyntaxError }) {
  return (
    ...
    >
      <div style={{ fontWeight: 'bold', color: '#c00', marginBottom: '0.5rem' }}>
...
export function ContentInput({
  ...
  const [syntaxError, setSyntaxError] = useState<MarkdownSyntaxError | null>(null);
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        if (err.code === 'ECONNABORTED' || err.message?.includes('timeout')) {
          ...
          setError('Processing took too long. Please try with shorter or simpler content.');
        } else if (err.response?.status === 400) {
          setError(err.response.data?.detail || 'Invalid input. Please check your text.');
        } else {
...
- More content here`}
          ...
          <div style={{ marginTop: '1rem' }}>
            <MarkdownPreview markdownContent={markdownContent} debounceMs={1000} />
          </div>
        </>
      )}
      ...
      {source === 'extracted' && extractedSlides && (
        <div style={{ marginTop: '1rem' }}>
          <p>Using {extractedSlides.length} slides from extracted PPTX content</p>
          <button type="button" onClick={handleExtractedContent} className="btn-primary">
