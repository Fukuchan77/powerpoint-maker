...
describe("App Integration", () => {
  ...
  it("completes the full flow: upload -> content input -> generate", async () => {
    ...
    expect(
      screen.queryByText(/2. Choose Content Source/i),
    ...
    vi.mocked(axios.post).mockResolvedValue(
      parseMarkdownResponse as AxiosResponse,
    );
  ...
  it("handles extraction flow", async () => {
    ...
    vi.mocked(axios.post).mockResolvedValueOnce(
      extractResponse as AxiosResponse,
    );
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Using 1 slides from extracted PPTX content/i),
      ).toBeInTheDocument();
    });
