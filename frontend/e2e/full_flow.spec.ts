...
test('complete user flow: upload, generate, and download', async ({ page }) => {
  ...
  await test.step('Upload and analyze template', async () => {
    ...
    const buffer = Buffer.from([0x50, 0x4b, 0x03, 0x04, 0x14, 0x00, 0x08, 0x00, 0x08, 0x00]); // Minimal fake zip signature (PK header)
    ...
    await expect(page.getByPlaceholder('e.g., The Future of AI in Healthcare')).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  await test.step('Generate and download presentation', async () => {
    ...
    await page.route('**/api/generate', async (route) => {
      await route.fulfill({
        ...
        body: Buffer.from([0x50, 0x4b, 0x03, 0x04, 0x14, 0x00, 0x08, 0x00, 0x08, 0x00]), // Minimal zip (PK header)
      });
    });
